...
const static Scalar g_colors[] = { CV_RGB(0, 0, 255),
	CV_RGB(0, 128, 255),
	CV_RGB(0, 255, 255),
	CV_RGB(0, 255, 0),
	CV_RGB(255, 128, 0),
	...
	CV_RGB(255, 0, 0),
	CV_RGB(255, 0, 255) };
static LBFRegressor *p_regressor;
static CascadeClassifier g_cascade;
void InitFaceAligiment(){
	extern string modelPath;
	extern string cascadeName;
	ReadGlobalParamFromFile(modelPath + "LBF.model");
	if (!g_cascade.load(cascadeName)){
		cerr << "ERROR: Could not load classifier cascade" << endl;
	}
	p_regressor = new LBFRegressor;
	p_regressor->Load(modelPath + "LBF.model");
}

void FeaturePoint(Mat img_in, vector<Point> &point_list){
	Mat img;
	img_in.copyTo(img);
	...
	t = (double)cvGetTickCount();
	g_cascade.detectMultiScale(smallImg, faces,
		1.1, 2, 0
		//|CV_HAAR_FIND_BIGGEST_OBJECT
		//|CV_HAAR_DO_ROUGH_SEARCH
		| CV_HAAR_SCALE_IMAGE
	...
	if (tryflip){
		flip(smallImg, smallImg, 1);
		g_cascade.detectMultiScale(smallImg, faces2,
			1.1, 2, 0
			//|CV_HAAR_FIND_BIGGEST_OBJECT
			//|CV_HAAR_DO_ROUGH_SEARCH
			| CV_HAAR_SCALE_IMAGE
	...
	for (vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++){
		...
		t = (double)cvGetTickCount();
		Mat_<double> current_shape = p_regressor->Predict(gray, boundingbox, 1);
		t = (double)cvGetTickCount() - t;
		printf("alignment time = %g ms\n", t / ((double)cvGetTickFrequency()*1000.));
		// draw bounding box
		rectangle(img, cvPoint(boundingbox.start_x, boundingbox.start_y),
		...
		point_list.clear();
		for (int i = 0; i < global_params.landmark_num; i++){
			point_list.push_back(Point2d(current_shape(i, 0), current_shape(i, 1)));
		}
		//        for(int i = 36;i <= 41;i++){
		//            circle(img,Point2d(current_shape(i,0),current_shape(i,1)),1,Scalar(0,0,255),3,8,0);
		//        }
